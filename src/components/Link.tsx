...
export default function Link({ href, isRTL = false,  children }: LinkProps) {
    return (
        <a href={href} className={`flex ${isRTL ? "flex-row-reverse gap-2" : "flex-row"} text-blue-500 group items-center`}>
            ...
            <a className="group-hover:underline">{children}</a>
        </a>
    )
}